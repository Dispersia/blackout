 ...
 * therefore the default handler for factory data and software update had to be
 * overriden. On other DK's only UAT is handled by the application.
 */
...
/* In this case we need to run only Bluetooth LE SMP advertising if it is
 * available */
          ...
          /* In this case we start both Bluetooth LE SMP and Matter advertising
           * at the same time */
...
void ButtonTimerEventHandler() {
  if (sBtnState == ButtonState::SoftwareUpdate) {
    ...
    /* Start timer for sFactoryResetTimeout to allow user to cancel, if
     * required. */
    ...
    /* If we reached here, the button was held past FactoryResetTriggerTimeout,
...
void AppTask::UpdateTemperatureMeasurement() {
  ...
  if (result == 0) {
    ...
    int result = sensor_channel_get(sBme688SensorDev, SENSOR_CHAN_AMBIENT_TEMP,
                                    &temperature);
    if (result == 0) {
      /* Defined by cluster temperature measured value = 100 x temperature in
       * degC with resolution of 0.01 degC. val1 is an integer part of the value
       * and val2 is fractional part in one-millionth parts. To achieve
       * resolution of 0.01 degC val2 needs to be divided by 10000. */
      mCurrentTemperature = static_cast<int16_t>(temperature.val1 * 100 +
                                                 temperature.val2 / 10000);
      LOG_DBG("New temperature measurement %d.%d *C", temperature.val1,
              temperature.val2);
    } else {
      LOG_ERR(
          "Getting temperature measurement data from BME688 failed with: %d",
          result);
    }
  } else {
...
#else
  /* Linear temperature increase that is wrapped around to min value after
...
CHIP_ERROR AppTask::StartApp() {
  ...
  Protocols::InteractionModel::Status status =
      Clusters::TemperatureMeasurement::Attributes::MinMeasuredValue::Get(
          kTemperatureSensorEndpointId, val);

  if (status != Protocols::InteractionModel::Status::Success || val.IsNull()) {
    LOG_ERR("Failed to get temperature measurement min value %x",
            to_underlying(status));
    return CHIP_ERROR_INCORRECT_STATE;
  }
  ...
  status = Clusters::TemperatureMeasurement::Attributes::MaxMeasuredValue::Get(
      kTemperatureSensorEndpointId, val);

  if (status != Protocols::InteractionModel::Status::Success || val.IsNull()) {
    LOG_ERR("Failed to get temperature measurement max value %x",
            to_underlying(status));
    return CHIP_ERROR_INCORRECT_STATE;
  }
  ...
  k_timer_start(&mTimer, K_MSEC(kTemperatureMeasurementIntervalMs),
